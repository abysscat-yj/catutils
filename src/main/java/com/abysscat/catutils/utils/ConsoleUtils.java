...
public interface ConsoleUtils {
     ...
     * */
    static String getFormatOutputString(String content, int fontColor, int fontType, int backgroundColor){
     ...
     * @param fontColor, 字体颜色：30黑 31红 32绿 33黄 34蓝 35紫 36深绿 37白
     * */
    static String getColoredOutputString(String content, int fontColor){
     ...
     * @param backgroundColor, 字背景颜色：40黑 41红 42绿 43黄 44蓝 45紫 46深绿 47白
     * */
    static String getBackgroundColoredOutputString(String content, int backgroundColor){
        return String.format("\033[%dm%s\033[0m", backgroundColor, content);
    }

    /**
     * 能接受一个顺序标识，按顺序产生带颜色的输出字符串     * */
    static String orderedColorString(String content, int i){
        int tmpColor = 31 + (i % 7);
        return String.format("\033[%dm%s\033[0m", tmpColor, content);
    }

    static void main(String []args){
        System.out.println(getFormatOutputString("字体颜色为红色，背景色为黄色，带下划线", 31, 3, 43));
        //按顺序输出各个颜色代码的字符串
        for(int i = 0; i < 7; i++){
